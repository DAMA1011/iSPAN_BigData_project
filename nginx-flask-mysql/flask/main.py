...
def map():
    ...
    for i in range(len(df)):
        ...
        folium.PolyLine(points,
                        ...
                        dash_array='4, 8',
                        popup='Line').add_to(m)
    with open("taipei-region.json", encoding='utf-8') as file:
        data = json.load(file)
    ...
    m.save(f'./templates/{map_html}')
    # m.save('./templates/parts/test_map4.html')
    df_len = df.shape[0]
    all = {"df": df, "route": final_url, "start": start,
            "end": end, "df_len": df_len, "map_name": map_html}
    return render_template('map.html', df=df, route=final_url, start=start, end=end, map_name=map_html)


def map_name():
    ran_num = str(uuid.uuid4())
    map_html = "map/"+ran_num+".html"

    return map_html


@app.route('/final_map')
def final_map(map_html):
    return render_template(map_html, title='Map')
